package devdojo.spring.springboot2.integration;
...
import devdojo.spring.springboot2.repository.DevDojoUserRepository;
...
import devdojo.spring.springboot2.wrapper.PageableResponse;
...
public class AnimeControllerIT {
    ...
    private static final DevDojoUser USER = DevDojoUser.builder()
            .name("devdojo")
            .password("{bcrypt}$2a$10$jknS/VP5nXKRBXIzOLq7uOD3RT4wYx1.DxcwSt6tmxccBqn6MCl3m")
            .username("devdojo")
    ...
    static class Config{
        ...
        public TestRestTemplate testRestTemplateRoleUserCreator(@Value("${local.server.port}") int port){
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    ...
                    .basicAuthentication("devdojo", "testbd");
        ...
        public TestRestTemplate testRestTemplateRoleAdminCreator(@Value("${local.server.port}") int port){
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    .rootUri("http://localhost:" + port)
                    .basicAuthentication("matheus", "testbd");
            return new TestRestTemplate(restTemplateBuilder);
        }
    }
    ...
    void findByName_ReturnsEmptyListOfAnimes_WhenAnimeIsNotFound(){
        devDojoUserRepository.save(USER);
        List<Anime> animes = testRestTemplateRoleUser.exchange("/animes/find?name=dbgt", HttpMethod.GET, null, new ParameterizedTypeReference<List<Anime>>(){
        }).getBody();
...
//        Assertions.assertThat(animeResponseEntity.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
    ...
    @Test
    @DisplayName("Delete returns 403 when when user is not admin")
